...
std::ofstream out;
out.open("./nacie_tr1bundle_h90.pts");
...
// end assembly grid
        ...
        // nacie esagonal grid
    // central rod
    double eps=1.e-9;double h=2.;int ndiv=90;
    double p=8.39972*1.e-3;
    //central rod
    std::vector<double> xcenters={0.,0.022717*3/2,0.022717*3/2};
    std::vector<double> ycenters={0.,0.022717*sqrt(3)/2,-0.022717*sqrt(3)/2};
    for(int icenter=0; icenter< xcenters.size();icenter++){
        std::vector<double> p1={xcenters[icenter],ycenters[icenter],eps}; double pt1d=0.;
        std::vector<double> p2={xcenters[icenter],ycenters[icenter],h -eps }; double pt2d=0.;
        ...
        for (int iring=0; iring<2;iring++)
        {
            ...
            double r=(iring+1)*p;
            double xc=xcenters[icenter], yc=ycenters[icenter];    
            for(int k=0;k<ntheta;k++){
