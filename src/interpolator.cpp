...

{
   ...
   std::cout<<"INTERPOLATOR::INTERPOLATOR mf_1 nb dof "<<mf_1_->nb_dof()<<std::endl;
   // std::cout<<"INTERPOLATOR::INTERPOLATOR Radius "<<radius_[0];
...
void INTERPOLATOR::build_operator(){
   ...
   for (size_type i = 0; i < nb_dof_v; i++){
	 ...
	 std::cout << "*"; std::cout.flush();
      ...
      std::cout <<"before int ";
      // getfem::mesh_trans_inv mti(mf_1_->linked_mesh(),0);
      ...
      std::cout<<((radius_)[0])[0] <<std::endl;
      std::vector<bgeot::base_node> pts_circle;
      // ... then parametrize the circle:
      for (size_type j = 0; j < NInt_; j++){ 	
         mti.add_point( mf_2_->point_of_basic_dof(i) + 
            ((radius_)[0])[0]*(cos(2*Pi*j/NInt_)*v1 + sin(2*Pi*j/NInt_)*v2) );
         pts_circle.push_back( mf_2_->point_of_basic_dof(i) + 
            ((radius_)[0])[0]*(cos(2*Pi*j/NInt_)*v1 + sin(2*Pi*j/NInt_)*v2) );
         }
         
//          std::cout <<"circle points "<< pts_circle<<std::endl;
         
         std::cout << "Before distribute "<<std::endl;
         //{
         mti.distribute();
         //}
         std::cout << "Mti nb of points "<<  mti.nb_points()<<std::endl; 
         
//          counting global dof
         dal::bit_vector bv_cv = mf_1_->linked_mesh().convex_index();
         size_type i_cv = 0; int checksum=0;
         for (i_cv << bv_cv; i_cv != size_type(-1); i_cv << bv_cv) {
//            std::cout << " element "<< mti.nb_points_on_convex(i_cv)<< " ";
           checksum+= mti.nb_points_on_convex(i_cv); 
        }
//              std::cout <<"total convex "<<checksum<<std::endl;         
		// Get the local interpolation matrix Mbari
		sparse_matrix_type Mbari(NInt_, nb_dof_t); gmm::clear(Mbari);
                getfem::interpolation(*mf_1_, mti, Pt, Pbari, Mbari, 1,1.e-8);
