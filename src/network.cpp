...
NETWORK::NETWORK (const GetPot& df): mim_(mesh_),
                                     mf_(mesh_),
                                     mf_coef_(mesh_),
                                     BC_value_(1),th_cond_f_(1),
                                     rhocp_f_(1), tau_(1), source_(1), pirr_(1)
{
  ...
  bgeot::pgeometric_trans pgt = 
		bgeot::geometric_trans_descriptor(df("oned/mesh/mesh_type", "GT_PK(1,1)"));
  getfem::pfem pf = getfem::fem_descriptor(df("oned/fem/fem", "FEM_PK(1,1)"));
  getfem::pintegration_method ppi = getfem::int_method_descriptor(df("oned/fem/integr_method", "IM_GAUSS1D(6)"));
  ...
  for (int i=0; i< radius_.size(); i++) radius_[i]=df("oned/geometry/radius", 0.1);
  delta_clad_=df("oned/geometry/delta_clad", 0.001);
  
  // materials
  rho_f_ =df("oned/material/UO2/rho",1.);
  k_f_=df("oned/material/UO2/k",1.);
  cp_f_=df("oned/material/UO2/cp",1.);
  std::cout << "uranium "<< rho_f_ << " " << k_f_<< " "<<cp_f_<<std::endl; 
  rho_cl_ =df("oned/material/Zircalloy/rho",1.);
  k_cl_=df("oned/material/Zircalloy/k",1.);
  cp_cl_=df("oned/material/Zircalloy/cp",1.);
  std::cout << "Cladding "<< rho_cl_ << " " << k_cl_<< " "<<cp_cl_<<std::endl; 
  
  double k_c=df("oned/material/LBE/k",1.);
  lref_=1.;
  Temp_ref_=df("oned/source",1.)*lref_*lref_/k_c;
  uref_=k_c/lref_;
  time_ref_=lref_*lref_/k_c;
  
  gmm::resize(Y_, mf_coef_.nb_dof());
  for(int ipt=0; ipt<mf_coef_.nb_dof(); ipt++)
      Y_[ipt]=1.*2* 3.1415 * k_cl_ / (log((radius_[0]+delta_clad_)/(radius_[0])));
  std::cout <<"representative coupling " << Y_[0] << std::endl;
//   std::cin.ignore();
 std::fill(Y_.begin(), Y_.end(), 1.e+5);// coefficient mass matrix
...
void  NETWORK::build_mesh(getfem::mesh& mesh, const GetPot& df){
   ...
   std::cout<<df("oned/mesh/input_fn", "segment.pts")<<std::endl;
   std::ifstream ifs(df("oned/mesh/input_fn", "segment.pts")); 
...
void NETWORK::configure_wp(const GetPot& df){
 ...
 th_cond_f_[0] = k_f_;
 rhocp_f_[0]=rho_f_*cp_f_;
 
//  th_cond_f_[0] = k_f_/(rho_f_*cp_f_) *(df("oned/material/LBE/cp",1.) * df("oned/material/LBE/rho",1.)/ df("oned/material/LBE/k",1.));
 th_cond_f_[0] = k_f_/(rho_f_*cp_f_);
 
 workspace_.add_fixed_size_constant("K", th_cond_f_);
 workspace_.add_fixed_size_constant("rhocp", rhocp_f_);
 tau_[0] = df("time/dt", 1.0 ); // dt into  the workspace
 source_[0]= df("oned/source", 1.0 )* df("oned/ConversionPowerFactor", 1.0 );//used for comparison with homogeneous
// source_[0]= df("oned/source", 1.0 );
 std::cout<< "Source is "<< source_[0]<<std::endl;
 // pirr_[0] = M_PI*radius_[0]*radius_[0];
 pirr_[0] = 1.;
 std::cout << M_PI << " " << radius_[0]<<std::endl;
 std::cout<< "pi rr "<< pirr_[0]<<std::endl;
 workspace_.add_fixed_size_constant("tau", tau_);
 workspace_.add_fixed_size_constant("source", source_);
 workspace_.add_fixed_size_constant("pirr", pirr_);
}
...
void NETWORK::assembly_mat(){
   ...
   // ------------------ expressions --------------------------
   workspace_.add_expression( "pirr*p.Test_p*rhocp + pirr*tau*K*Grad_p.Grad_Test_p*rhocp", mim_);
     ...
     if ((BCList_[i].label).compare("DIR")==0)
        {  int counter=0;
           ...
           if (local_idx_bc_[i]==counter) {
                  K_(ipt,ipt)+=1.e+8;
                  //std::cout << "adding bc on "<< ipt <<" at "<< local_idx_bc_[i] <<std::endl;
               }
           counter++;
           }
       }
...
void NETWORK::assembly_rhs(){
   ...
   gmm::clear(rhs_);
   // ------------------ expressions --------------------------
//    workspace_.add_expression( "+source*Test_p*tau/rhocp + p_old.Test_p", mim_);
   workspace_.add_expression( "+pirr*source*Test_p*tau*(Heaviside(X(1)*X(1)+X(2)*X(2) - 0.0227*0.0227)) + pirr*p_old.Test_p*rhocp", mim_);
